...
const EditUser = () => {
  ...
  const fetchUser = async () => {
    try {
      ...
      setFormData({ name: user.name || '', email: user.email || '', role: user.role || 'accountant' });
    } catch (error) {
      toast.error(getErrorMessage(error, lang === 'ar' ? 'فشل تحميل بيانات المستخدم' : 'Failed to load user data'));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      setSubmitting(true);
      await axiosInstance.patch(`/user/${id}`, { name: formData.name, email: formData.email, role: formData.role });
      toast.success(lang === 'ar' ? 'تم تحديث المستخدم بنجاح!' : 'User updated successfully!');
      setTimeout(() => navigate('/users'), 1500);
    } catch (error) {
      toast.error(getErrorMessage(error, lang === 'ar' ? 'فشل تحديث المستخدم' : 'Failed to update user'));
    } finally {
      setSubmitting(false);
    }
  };

  if (loading) return <FullPageLoader text={lang === 'ar' ? 'جاري تحميل البيانات...' : 'Loading data...'} />;
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5">
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-xl p-4">
              <p className="text-sm text-gray-500">
                <span className="font-medium text-gray-700">{lang === 'ar' ? 'ملاحظة: ' : 'Note: '}</span>
                {lang === 'ar'
                  ? 'لا يمكن تغيير كلمة المرور من هنا. يرجى التواصل مع المسؤول.'
                  : 'Password cannot be changed here. Please contact the administrator.'}
              </p>
            </div>
