...
export default function SupplierPaymentsReport({ isAr }) {
  ...
  const supplierPaymentRows = useMemo(() => {
    ...
    const refRows = rawRefunds.map(r => ({
      ...
      paymentNo: `${isAr ? 'استرداد' : 'Refund'} #${r.refundNo}`,
      ...
      <div className="flex-1 min-w-0"
        ...
        {hasLoaded && !loading && (
          <>
            ...
            <ReportsTable columns={columns} filteredData={filteredData} allData={supplierPaymentRows} isAr={isAr} activeTab="supplierPayments" renderCell={renderCell} />
          </>
        )}
      </div>
