...
export default function ProfitsReport({ isAr }) {
  ...
  const statistics = useMemo(() => {
    ...
    return [
      ...
      { label: isAr ? 'عدد الدفعات'      : 'Payment Count',      value: rawClientPayments.length, color: 'orange', icon: PiggyBank },
    ];
  }, [rawClientPayments, isAr]);
      ...
      <div className="flex-1 min-w-0"
        ...
        {!loading && !error && !hasLoaded && (
          <div className="flex flex-col items-center justify-center py-24 text-gray-400">
            <PiggyBank className="w-14 h-14 mb-4 opacity-30" />
        ...
        {hasLoaded && !loading && (
          <>
            ...
            <ReportsTable columns={columns} filteredData={filteredData} allData={profitRows}
              isAr={isAr} activeTab="profits" renderCell={renderCell} />
          </>
        )}
      </div>
