...
export default function SupplierLedgerReport({ isAr }) {
  ...
  const getSupplierName = (id) => { const s = suppliers.find(x => x._id === id); if (!s) return id || '—'; return isAr ? (s.nameAr || s.name) : (s.name || s.nameAr); };
  ...
  const handleExportExcel = () => { const h = columns.map(c => ({ [c.key]: isAr ? c.labelAr : c.labelEn })); exportToExcel(filteredData, h, isAr ? 'دفتر الموردين' : 'Supplier Ledger', isAr ? 'ar' : 'en'); };
  const handleExportPDF   = () => { const h = columns.map(c => ({ [c.key]: isAr ? c.labelAr : c.labelEn })); const t = isAr ? 'دفتر الموردين' : 'Supplier Ledger'; exportToPDF(filteredData, h, t, isAr ? 'ar' : 'en', t); };
